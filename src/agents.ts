...
export function renderProjectCustomAgentToml(role: StudioRoleId, config: ProjectConfig, engines: EngineConfigRegistry): string {
  ...
  const body = [
    ...
    `developer_instructions = ${tomlMultiline([
      ...
      `Before broad file reads, run \`npm run ctx:role -- ${role}\` and \`npm run ctx:changed\` when the npm scripts are available; then inspect only the surfaced task-relevant files.`,
      "",
      pkg.systemPrompt,
      "",
...
export function renderProjectRolePrompt(role: StudioRoleId, config: ProjectConfig, engines: EngineConfigRegistry): string {
  ...
  const body = [
    ...
    "",
    `Run \`npm run ctx:role -- ${role}\` and \`npm run ctx:changed\` before broad inspection when the npm scripts are available; then inspect only the surfaced task-relevant files.`,
    "",
    "## Project Summary",
    "",
    config.project.concept,
