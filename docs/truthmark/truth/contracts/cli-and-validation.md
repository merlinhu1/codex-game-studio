...
- Approval grant output includes the new approval ID, normalized role, objective/hash information, scopes, and the approval-store path for inspection.
...
- Decision (2026-06-13): Expose approval grant/list/revoke as audit-store management commands only; do not wire approvals into `run <role>` or `task run` eligibility in this slice.
...
- This contract does not define runtime approval enforcement for `run <role>` or `task run`.
