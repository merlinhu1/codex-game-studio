...
function makeRoot(): string {
  ...
  const skillBody = (name: string, tier: string, model: string, effort: string, hash: string) => `---\nname: ${name}\ndescription: ${name}\nmodel_tier: ${tier}\nmodel: ${model}\nmodel_reasoning_effort: ${effort}\nargument-hint: describe target\nprimary-agent: producer\ntool-policy: read/edit/shell\nisolation: repository-root\nsource-reference: local\nsource-hash: ${hash}\n---\n\n# ${name}\n\n## Objective\n\nObjective.\n## Inputs\n\nInputs.\n## Arguments\n\nArgs.\n## Procedure\n\nProcedure.\n## Output Contract\n\nOutput.\n## Quality Gates\n\nQuality.\n## Decision Gates\n\nGates.\n## Handoff\n\nHandoff.\n`;
  writeFileSync(path.join(root, ".agents", "skills", "cgs-bugfix", "SKILL.md"), skillBody("cgs-bugfix", "terra", "gpt-5.6-terra", "high", "b".repeat(64)));
  writeFileSync(path.join(root, ".agents", "skills", "cgs-standards-gameplay", "SKILL.md"), skillBody("cgs-standards-gameplay", "luna", "gpt-5.6-luna", "low", "c".repeat(64)));
...
describe("prompt surface validation", () => {
  ...
  test("rejects prompt surfaces whose declared tier and exact model drift", () => {
    const root = makeRoot();
