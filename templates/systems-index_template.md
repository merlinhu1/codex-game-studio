...
Systems index scaffold for system inventory, priority tiers, dependency map, owners, and coverage gaps.

Use this Codex-native template when a game-studio task needs a reviewable `systems-index` artifact. Keep claims tied to source files, playtest observations, engine output, or explicit user decisions.
...
- Artifact body: structured sections appropriate for `systems-index` with assumptions labeled.
