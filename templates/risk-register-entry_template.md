...
Risk entry scaffold for probability, impact, triggers, mitigation, owner, and status.

Use this Codex-native template when a game-studio task needs a reviewable `risk-register-entry` artifact. Keep claims tied to source files, playtest observations, engine output, or explicit user decisions.
...
- Artifact body: structured sections appropriate for `risk-register-entry` with assumptions labeled.
